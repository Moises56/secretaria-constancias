...
interface ConstanciaListPaginationProps {
  page: number;
  totalPages: number;
  total: number;
  pageSize: number;
...
export function ConstanciaListPagination({
  page,
  totalPages,
  total,
  pageSize,
}: ConstanciaListPaginationProps) {
  ...
  return (
    <div
      data-testid="constancia-pagination"
    ...
    >
      <p data-testid="pagination-summary">
        ...
        <span className="text-foreground font-medium">{total}</span> constancias
      </p>
      ...
      <div className="flex items-center gap-3">
        ...
        >
          <SelectTrigger className="h-8 w-[80px]" data-testid="page-size">
            <SelectValue />
          </SelectTrigger>
          <SelectContent>
            {PAGE_SIZES.map((s) => (
